(function () {
  ...
  function bindSecretGesture() {
    const activePointers = new Map(); // id -> {startY, lastY}
    let holdTimer = null;
    let triggered = false;
    ...
    function startHoldIfReady() {
      clearHold();
      if (activePointers.size === 2) {
        holdTimer = setTimeout(() => {
          if (activePointers.size === 2 && !triggered) {
            triggered = true;
            soundConfirm();
            openSecret();
          }
    ...
    document.addEventListener("pointerdown", (e) => {
      activePointers.set(e.pointerId, { startY: e.clientY, lastY: e.clientY });
      if (activePointers.size !== 2) triggered = false;
      startHoldIfReady();
    });
    document.addEventListener("pointermove", (e) => {
      ...
      if (activePointers.size === 2 && !triggered) {
        ...
        const totalDelta = pts.reduce((sum, pt) => sum + (pt.startY - pt.lastY), 0) / pts.length;
        if (totalDelta > 70) { // moved up enough
          triggered = true;
          clearHold();
          soundConfirm();
          openSecret();
